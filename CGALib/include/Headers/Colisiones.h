...
bool intersectionSegmentAABB(glm::vec3 origen, glm::vec3 target, AbstractModel::AABB aabb) {
	float tmin = -FLT_MAX;
	float tmax = FLT_MAX;
	...
	//si se llega en este punto hay collision el rayo y la caja

	if (tmin >= 0 && tmin <= glm::distance(origen, target)) {
		return true;
	}
	//esto es para detectar colision dcon mayow
	if (tmin >= 0 && tmax <= glm::distance(origen, target)) {
		return true;
	}
	return false;
...
bool rayOBBIntersect(glm::vec3  origen,glm::vec3 target, AbstractModel::OBB obb) {
	...
	return intersectionSegmentAABB(origenAlineado, targetAlineado, aabb);
}
